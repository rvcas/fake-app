...
export async function handleAuthOptions(request: Request, env: Env): Promise<Response> {
...
export async function handleAuthVerify(request: Request, env: Env): Promise<Response> {
  ...
  const { challenge, credentialId: storedCredentialId } = JSON.parse(stored) as {
    challenge: string;
  ...
  if (!credentialId) {
    return Response.json({ error: "No credential ID in response" }, { status: 400 });
  }
